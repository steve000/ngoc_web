<template>
    <div class="create-sales-order saleModuleCreate" style="padding: 20px 30px;">
        <div class="content">
            <!-- 新增订单 -->
            <div class="title">
                <div>基本信息</div>
            ...
            <div class="content-wrapper">
                <el-form label-width="110px" :model="createOrderParams" label-position="left" :rules="createOrderParamsRules" ref="createOrder">
                    <div class="displayStyle">
                        <el-form-item label="操作人：" required>
                            <el-input v-model="userName" disabled class="row_width"></el-input>
                        </el-form-item>
                    </div>
                    <div class="displayStyle displayStyle1">
                        <el-form-item label="客户：" prop="channelId">
                            <el-select v-model="createOrderParams.channelId" filterable placeholder="请选择客户" class="row_width" @change="selectedChannel">
                                <el-option v-for="(item, index) in channelTypeOptions" :key="index" :label="item.channelName" :value="item.channelId"></el-option>
                            </el-select>
                        </el-form-item>
                    </div>
                    <div>
                        <el-form-item label="备注：">
                            <el-input type="textarea" style="width:100%;" :rows="4" :maxlength="200" v-model="createOrderParams.remark" resize="none"></el-input>
                        </el-form-item>
                    </div>
                </el-form>
            </div>
        ...
        <div class="content">
            ...
            <div class="content-wrapper">
                <!-- <el-button plain size="mini" @click="addReceiverAddress = true" class="mb20">
                    <i class="el-icon-plus"></i> 新增{{createOrderParams.channelId === '-1' ? '客户' : '收货信息' }}
                </el-button> -->
                <el-table :data="receivingInformation" :height="receivingInformation.length ? '' : '100'" style="border: 1px solid #f5f5f5;border-top: 0;">
                    <el-table-column label="" width="48" v-if="createOrderParams.channelId !== '-1'">
                    ...
                    <el-table-column prop="linkMan" label="收货人" width="200" show-overflow-tooltip></el-table-column>
                    <el-table-column label="收货地址" show-overflow-tooltip>
                        <template slot-scope="scope">
                            {{ `${scope.row.province}-${scope.row.city}-${scope.row.area} ${scope.row.address}` }}
                        </template>
                    </el-table-column>
                    <el-table-column prop="phone" label="联系电话" width="250" show-overflow-tooltip></el-table-column>
                    <el-table-column label="操作" align="left" width="300">
                        <template slot-scope="scope">
                            <a @click="editorAddress(scope.row, scope.$index)" class="createAddress_func">编辑</a>
                            <a class="createAddress_func" @click="deleteAddress(scope.row)">删除</a>
            ...
            <div class="pl20 pb20">
                ...
                <el-button plain @click="quit(routesMapping.SalesOrder, true)">取消</el-button>
            </div>
        </div>
    </div>
</template>
...
export default {
    ...
    data() {
        return {
            ...
            createOrderParamsRules: {
                channelId: [{ required: true, message: '请选择客户', trigger: 'change' }]
            },
    ...
    methods: {
        ...
        submitCollectAddressParams(params) {
            ...
            if (!(this.createOrderParams.channelId === '-1')) {
            ...
            this.$confirm('确认删除?', '提示', {
                ...
                if (item.channelAddressId || item.channelAddressId === 0 || this.createOrderParams.channelId !== '-1') {
                    this.updateChannelAddressStatus({ channelAddressId: item.channelAddressId, status: 2 }).then(res => {
                        if (res.code === 0) {
                            this.selectedChannel(this.createOrderParams.channelId)
                            this.$message({
                                type: 'success',
                                message: '删除成功'
                            });
            ...
            this.$refs['createOrder'].validate((valid) => {
                if (valid && this.commodityInformationList.length) {
                    ...
                    if (!this.commodityInformationList.some(i => i.saleCount === undefined || i.price === undefined || i.skuName === undefined)) {
                        ...
                        this.createOrderAdd(this.createOrderParams).then(res => {
                            this.$message({
                                type: 'success',
                                message: `添加成功`
                            })
                            ...
                            this.createOrderParams.pluList = undefined
                            this.hidePageLoading()
                        })
                    } else {
                        this.$message({
        ...
        selectedAddress(val) {
            ...
            if (cur.channelAddressId) {
                ...
                Object.assign(this.createOrderParams, cur, { custAddressId: cur.channelAddressId }, { channelId: '-1' })
            }
        },
        // 初始化
        init() {
        ...
        queryChannel() {
            this.queryChannelBySearch({ searchContent: '' }).then(res => {
                res.result.unshift({ channelId: '-1', channelName: '其他' })
