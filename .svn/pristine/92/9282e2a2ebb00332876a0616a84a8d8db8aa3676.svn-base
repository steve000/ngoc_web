            ...
            <div class="viewLeftTop">
                <el-row>
                    <el-col :span="9">
                        <h4 class="order">
                            <span>经销对账单号：</span>
                            <span >{{billDetail.payCheckNo}}</span>
                        </h4>
                    </el-col>
                    <el-col :span="9">
                        <h4 class="order">
        ...
        <div class="DetailsRightBox">
            ...
            <el-form :modle="billDetail" label-position="left" class="TotalBasic">
                <!-- <el-form-item label="供应商名称：">
                    <span>{{billDetail.payGroupName ? billDetail.payGroupName : '--'}}</span>
                </el-form-item> -->
                <el-form-item label="付款组织：">
                    <span>{{billDetail.payUserGroupName ? billDetail.payUserGroupName : '--'}}</span>
                </el-form-item>
...
<script>
    ...
    export default {
        ...
        mounted(){
            ...
            this.payCheckNo = this.$route.params.payCheckNo;
            this.queryData()
        },
        methods: {
            ...mapActions(["showPageLoading", "hidePageLoading", "payCheckInfo"]),
            getRadioValue(value){
            },
            queryData(){
                this.showPageLoading();
                this.payCheckInfoFunction({id: this.id});
            },
            formatMomentData(date){
                return utils.formatMomentData(date)
            },
            payCheckInfoFunction(param){
                this.payCheckInfo(param).then((resp) => {
