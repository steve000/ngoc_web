...
<script>
   ...
   export default {
        ...
        mixins: [MessageMixins, TableHeightMixins, PlatformMixins],
        components: {
        ...
        watch: {
            flag: function (nval) { 
                ...
                this.param.pageNo = 1;
                console.log(this.flag,this.type)
                if (this.type === 'all') {
        ...
        methods: {
            ...mapActions(["reducePromotionList",'reducePromotionDelete','reducePromotionChangeApp','reducePromotionChange', 'updatePurchaseOrderComment', "showPageLoading", "hidePageLoading"]),
            ...
            refreshPage(param = this.param) {
                ...
                this.reducePromotionListFunction(param);
            },
            initData() {
                this.getReset();
            },
            ...
            reducePromotionListFunction(param) {
                this.reducePromotionList(param).then((resp) => {
                    this.hidePageLoading();
                    //console.log(resp)
                    this.approveStatus = resp.result.approveStatus
                   // this.approveStatusText = 
                    console.log(this.approveStatusText)
            ...
            viewPurchaseDetail(row) {
                this.$router.push(`/sales/promotion/reducePromotionDetails/${row.id}`)
            },
            editPurchaseOrderFunction(row) {
                console.log(row.id)
                this.$router.push(`/sales/promotion/reducePromotionEdit/${row.id}`)
            },
            deletePurchaseOrderFunction(row) {
                let params={id:row.id,promotionNo:row.promotionNo}
                this.$confirm('确认删除此条满减促销记录', '提示', {
                    confirmButtonText: '确定',
                    cancelButtonText: '取消',
                    type: 'warning'
                }).then(() => {
                    this.reducePromotionDelete(params).then((resp) => {
                        if (resp == 'success') {
            ...
            getButtonList() {
                ...
                if(this.approveStatus==1){
                    buttonArray = [{type: 'delete', text: '删除'}];
                }else{
                    buttonArray = [{type: 'edit', text: '编辑'}, {type: 'delete', text: '删除'}];
                }
                return buttonArray
            },
            ...
            changeAppStatus(row){
                ...
                this.reducePromotionChangeApp(params).then(res=>{})
            },
            changeStatus(row){
                let params={status:row.status,id:row.id}
                this.reducePromotionChange(params).then(res=>{})
            }
        }
    }
</script>
