<template>
    <div class="normal-promotion-list">
...
<script>
   ...
   export default {
        ...
        methods: {
            ...mapActions(["normalPromotionList",'normalPromotionDelete','normalPromotionChangeApp','normalPromotionChange', 'updatePurchaseOrderComment', "showPageLoading", "hidePageLoading"]),
            ...
            refreshPage(param = this.param) {
                ...
                this.normalPromotionListFunction(param);
            },
            initData() {
                this.getReset();
            },
            ...
            normalPromotionListFunction(param) {
                this.normalPromotionList(param).then((resp) => {
            ...
            viewPurchaseDetail(row) {
                this.$router.push({path: `/sales/promotion/normalPromotionDetails/${row.id}`})
            },
            editPurchaseOrderFunction(row) {
                this.$router.push({path: `/sales/promotion/normalPromotionEdit/${row.id}`})
            },
            deletePurchaseOrderFunction(row) {
                ...
                this.$confirm('确认删除此条普通促销记录', '提示', {
                    confirmButtonText: '确定',
                    cancelButtonText: '取消',
                    type: 'warning'
                }).then(() => {
                    this.normalPromotionDelete(this.deletePurchaseOrderParam).then((resp) => {
                        this.refreshPage();
                        this.$emit("closeEdit");
                        this.$emit("closeView");
                    });
            ...
            changeAppStatus(row){
                ...
                this.normalPromotionChangeApp(params).then(res=>{})
            },
            changeStatus(row){
                let params={status:row.status,id:row.id}
                this.normalPromotionChange(params).then(res=>{})
            }
        }
    }
</script>
