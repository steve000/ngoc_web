                ...
                <el-table-column label="单据号" prop="billNo" min-width="200">
                    <template slot-scope="scope">
                        <div v-if="scope.row.status == 0 && scope.row.approveStatus == 0" class="row-btn-show">
                            <div style="cursor:pointer" class="row-show" @click="$router.push({path: `/goods/manage/specialPricePolicy/${scope.row.id}/${scope.row.billNo}/${scope.row.approveStatus}`})">{{scope.row.billNo}}</div>
