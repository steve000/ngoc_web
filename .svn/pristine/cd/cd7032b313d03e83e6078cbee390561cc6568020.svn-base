            ...
            <div class="content-wrapper">
                <el-form label-position="right" label-width="130px" :model="param" :rules="rules" class="topContent">
                    ...
                    <el-form-item label="订单类型：" required>
                        <el-input class="form-input-width"  v-model="param.billType" size="small" :disabled="true"></el-input>
                    </el-form-item>
                    ...
                    <!-- </div> -->
                    <!-- <el-row> -->
                        <el-col :span="13" style="margin-top: 5px" prop="remarks">
                    ...
                    <table class="productTable native-table-show">
                        ...
                        <tbody>
                            ...
                            <td :title="one.distributionCount">
                                <el-input :placeholder="one.totalCount ? `0~${one.totalCount}` : '0'" class="table-input" size="small" type="number" v-model="one.distributionCount" :min="0" @keyup.native="getpurchaseTotalPrice(one)" @blur="getpurchaseTotalPrice(one)"></el-input>
                            </td>
                            <td :title="one.remark">
...
export default {
  ...
  data() {
    ...
    return {
      ...
      rules: {
        ...
        remarks:[{required: false, validator: validatorComments, trigger: 'change'}],
      },
      addressValidate: {
  ...
  methods: {
    ...
    getOutStorehouseType() {
      let postData = {
        userGroupId: Number(this.param.userGroupId),
        status: 0,
    ...
    queryProductDetail(val, index) {
      ...
      this.queryStockSkuPrice({skuId: val.skuId, storehouseId: this.createStockOut.storehouseId}).then((resp) => {
        if (resp.result.length < 1) {
          this.showWarning("此商品库存不足");
        } else {
          resp.result[0].distributionPrice = resp.result[0].price;
          var product = Object.assign({},this.checkskuList[index],resp.result[0])
          this.$set(this.checkskuList, index, product)
        }
      })
    ...
    getpurchaseTotalPrice(item) {
      ...
      if (item.distributionCount > item.totalCount) {
        this.showWarning("输入数量库存不足");
      }
      if (utils.isDigit(item.distributionCount)) {
        console.log("计算22");
          ...
          // console.log(item.distributionPrice, item.distributionCount, typeof(item.distributionPrice),typeof(item.distributionCount), "??/???/????");
          this.$set(item, "totalPrice", utils.mul(item.distributionPrice, item.distributionCount))
    ...
    submitForm() {
      ...
      this.creatData.billType = this.getBilltypeText(this.param.billType);
      ...
      this.distributionOrderAdd(this.creatData).then(resp => {
