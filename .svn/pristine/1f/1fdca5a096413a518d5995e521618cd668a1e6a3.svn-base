<template>
    <div class="create-storage">
                ...
                <el-form label-position="right" label-width="110px" :model="param" :rules="rules" class="topContent">
                    ...
                    <el-form-item label="配送实体：" prop="disUserGroupName">
                        <el-select v-model="param.disUserGroupName"
                                   ...
                                   :filter-method="disUserGroupNameFilter"
                        ...
                        <div class="displayStyle">
                            <el-form-item label="收货人：" prop="linkman" >
                                <el-input v-model="createStockIn.linkman" size="small"
                                          placeholder="请输入联系人" :maxlength="20"  disabled></el-input>
                            </el-form-item>
                        </div>
                        <div class="displayStyle">
        ...
        <div class="btn-section pt20">
            ...
            <el-button  size="small" class="cancel-button" @click="quit(routesMapping.requireIndex,true)">取消</el-button>
        </div>
    </div>
</template>
...
export default {
  ...
  mounted() {
    ...
    console.log(this.paramsData,"带过来的参数")
    this.id = this.$route.params.id;
    this.requireNo = this.$route.params.requireNo;
    this.queryData(); //初始化根据参数拿到详细数据
    this.getInStorehouseType();  // 要货实体ID查询 （登录即固定不变）
    this.getAllGroupList(); //拿到配送实体
  },
  methods: {
    ...mapActions([
      "userGroupAll", //查询所有实体
      ...
      "requireOrderInfo", //拿到详细数据
      "requireOrderEdit", //要货编辑接口
      "showPageLoading",
      "hidePageLoading"
    ]),
    // 查询详细数据
    ...
    customGroupChoosed(val) {
      ...
      // this.getOutStorehouseType(val); //执行拿发货仓list
      this.getSkuListByUserGroup(val); //拿到配送实体下商品列表
    },
    // 客户实体过滤操作
    disUserGroupNameFilter(string) {
      if (!string) {
        return (this.allGroupList = this.filterAllGroupList);
      }
      this.allGroupList = this.filterAllGroupList.filter(item => {
        if (
          (item.userGroupId &&
            item.userGroupId.toString().indexOf(string) >= 0) ||
    ...
    chooseGoods() {
      if (!this.param.disUserGroupName) {
        this.showWarning("编辑带出配送实体");
        return false;
      }
    },
    // 选择要货数量
    ...
    submitForm() {
      ...
      this.creatData.requireNo = this.param.requireNo;
      ...
      for (let i = 0; i < this.checkskuList.length; i++) {
        ...
        if (checkskuListItem.requirePrice === "") {
      ...
      this.showPageLoading();
      this.requireOrderEdit(this.creatData)
        .then(resp => {
          this.hidePageLoading();
