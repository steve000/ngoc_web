        ...
        <div class="table-container">
            <div class="nav-top">
                <div class="search-container">
                    <search ref="searchParent" @searchValue="getSearchValue"
                            :isClear='isClear'
                            :showPlaceholder="'输入促销单号'"></search>
                </div>
            </div>
            <div class="table-wrapper" id="purchaseListTable">
                <el-table :data="purchaseList" border
                    ...
                    <el-table-column label="审批状态" prop="approveStatus" min-width="110">
                        <template slot-scope='scope'>
                            <el-select @change="changeAppStatus(scope.row)" v-model="scope.row.approveStatus">
...
<script>
   ...
   export default {
        ...
        methods: {
            ...mapActions(["combPromotionList",'combPromotionDelete','combPromotionChangeApp','combPromotionChange', 'updatePurchaseOrderComment', "showPageLoading", "hidePageLoading"]),
            ...
            refreshPage(param = this.param) {
                ...
                this.combPromotionListFunction(param);
            },
            initData() {
                this.getReset();
            },
            ...
            combPromotionListFunction(param) {
                this.combPromotionList(param).then((resp) => {
            ...
            viewPurchaseDetail(row) {
                this.$router.push(`/sales/promotion/combInfo/${row.id}`)
            },
            editPurchaseOrderFunction(row) {
                console.log(row.id)
                this.$router.push(`/sales/promotion/combEdit/${row.id}`)
            },
            deletePurchaseOrderFunction(row) {
                this.deletePurchaseOrderParam.purchaseId = row.id;
                this.deletePurchaseOrderParam.promotionNo = row.promotionNo;
                this.$confirm('确认删除此条促销记录', '提示', {
            ...
            getButtonList() {
                let buttonArray = [{type: 'delete', text: '删除'},{type: 'edit', text: '编辑'}];
                return buttonArray
            },
            ...
            changeAppStatus(row){
                ...
                this.combPromotionChangeApp(params).then(res=>{})
            },
            changeStatus(row){
                let params={status:row.status,id:row.id}
                this.combPromotionChange(params).then(res=>{})
            }
        }
    }
</script>
