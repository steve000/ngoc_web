<template>
    <div class="LocalStorageList">
        <div class="top-scroll-section">
            ...
            <div class="table-empty-height">
            	<el-table :data="skuTotalStockList" border>
                    ...
                    <el-table-column label="库存数量" prop="productTotalNum" min-width="100">
                        <template slot-scope="scope">
                            <div class="row-show" :title="scope.row.productTotalNum">{{scope.row.productTotalNum !== '' ? scope.row.productTotalNum : '--'}}
                            </div>
                        </template>
                    </el-table-column>
        ...
        <modal title="代发库存详情" v-model="isShowStockDetail" width="50%" class="modelStock">
                ...
                <el-table :data="LocalStockDetail">
                    ...
                    <el-table-column label="代发库存数量">
                        <template slot-scope="scope">
                            <div class="row-show" :title="scope.row.productNum">{{scope.row.productNum  !== '' ? scope.row.productNum : '--'}}
                            </div>
                        </template>
                    </el-table-column>
                </el-table>
...
<script>
    ...
    export default{
        ...
        methods:{
            ...
            LookProxyStock(skuId){//查看代发仓仓库详情
                let param = {
                    skuId : skuId,
                    stockType:3,
                    storehouseId:this.storehouseId
