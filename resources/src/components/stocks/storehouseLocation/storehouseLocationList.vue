    ...
    <div class="purchase-list">
        <div class="table-container">
            <div class="nav-top">
                ...
                <div class="search-container">
                    <search @searchValue="getSearchValue"
                            :isClear='isClear'
                            :showPlaceholder="'输入仓库编号/名称'"></search>
                </div>
            </div>
            ...
            <div class="table-wrapper" id="purchaseListTable">
                <el-table :data="storehouseLocationList" border
                          :height="tableHeight"
                          ref="storehouseLocationList"
                          :row-class-name="tableRowClassName">
                    <el-table-column label="仓位名称" prop="locationName" min-width="170">
                        <template slot-scope="scope">
                            <div class="row-btn-show">
                                <div class="row-show" :title="scope.row.locationName"
                                     @click="viewPurchaseDetail(scope.row)">{{scope.row.locationName}}
                                </div>
                    ...
                    <el-table-column label="排" prop="row" min-width="110"></el-table-column>
                    <el-table-column label="列" prop="columns" min-width="110"></el-table-column>
...
<script>
    ...
    export default {
        ...
        methods: {
            ...mapActions(["stockStorehouselocationQuerypagelist", "stockStorehouselocationDelete", "showPageLoading", "hidePageLoading"]),
            ...
            queryPurchaseOrderListFunction(param) {
                this.stockStorehouselocationQuerypagelist(param).then((resp) => {
                    this.hidePageLoading();
                    this.storehouseLocationList = resp.result;
                    this.totalCount = resp.totalCount;
                }).catch((err) => {
                    this.storehouseLocationList = [];
                    this.totalCount = 0;
                    this.hidePageLoading();
                });
            },
            ...
            deletePurchaseOrderFunction(row) {
                ...
                this.$confirm('确认删除', '提示', {
                    ...
                    this.stockStorehouselocationDelete(this.deletePurchaseOrderParam).then((resp) => {
                        this.refreshPage();
                        // this.$emit("closeEdit");
                        // this.$emit("closeView");
                    });
