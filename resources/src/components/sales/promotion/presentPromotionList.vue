<template>
    <div class="present-promotion-list">
...
<script>
   ...
   export default {
        ...
        methods: {
            ...mapActions(["presentPromotionList",'presentPromotionDelete','presentPromotionChangeApp','presentPromotionChange', 'updatePurchaseOrderComment', "showPageLoading", "hidePageLoading"]),
            ...
            refreshPage(param = this.param) {
                ...
                this.presentPromotionListFunction(param);
            },
            initData() {
                this.getReset();
            },
            ...
            presentPromotionListFunction(param) {
                this.presentPromotionList(param).then((resp) => {
            ...
            viewPurchaseDetail(row) {
                this.$router.push({path: `/sales/promotion/presentPromotionDetails/${row.id}`})
            },
            editPurchaseOrderFunction(row) {
                this.$router.push({path: `/sales/promotion/presentPromotionEdit/${row.id}`})
            },
            deletePurchaseOrderFunction(row) {
                ...
                this.$confirm('确认删除此条赠券促销记录', '提示', {
                    confirmButtonText: '确定',
                    cancelButtonText: '取消',
                    type: 'warning'
                }).then(() => {
                    this.presentPromotionDelete(this.deletePurchaseOrderParam).then((resp) => {
                        this.refreshPage();
                        this.$emit("closeEdit");
                        this.$emit("closeView");
                    });
            ...
            changeAppStatus(row){
                ...
                this.presentPromotionChangeApp(params).then(res=>{})
            },
            changeStatus(row){
                let params={status:row.status,id:row.id}
                this.presentPromotionChange(params).then(res=>{})
            }
        }
    }
</script>
