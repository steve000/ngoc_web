    ...
    <div class="purchase-list">
        <div class="table-container">
            ...
            <div class="table-wrapper" id="purchaseListTable">
                <el-table :data="vendorRegisterList" border
                          :height="tableHeight"
                          ref="vendorRegisterList"
                    ...
                    <el-table-column label="地址" prop="addressAll" min-width="110"></el-table-column>
                    <el-table-column label="注册时间" prop="registerTime" min-width="110"></el-table-column>
                    <el-table-column label="备注" prop="comments" min-width="130"></el-table-column>
    ...
    export default {
        ...
        methods: {
            ...
            queryPurchaseOrderListFunction(param) {
                this.purchaseVendorregisterQuerylistpage(param).then((resp) => {
                    this.hidePageLoading();
                    this.vendorRegisterList = resp.result;
                    this.totalCount = resp.totalCount;
                }).catch((err) => {
                    this.vendorRegisterList = [];
                    this.totalCount = 0;
                    this.hidePageLoading();
                });
            },
            ...
            viewPurchaseDetail(row) {
                this.$router.push({path: `/purchase/vendorRegisterDetails/${row.id}`})
            },
            ...
            deletePurchaseOrderFunction(row) {
                ...
                this.$confirm('确认删除此条供应商新品引进信息', '提示', {
                    confirmButtonText: '确定',
                    cancelButtonText: '取消',
                    type: 'warning'
                }).then(() => {
                    this.purchaseVendorregisterDelete(this.deletePurchaseOrderParam).then((resp) => {
                        this.refreshPage();
                    });
                }).catch((err) => {
                    this.$message({
            ...
            changeApproveStatus(row){
                ...
                this.purchaseVendorregisterChangeapprovestatus(params).then(res=>{
                    this.$message({
                        message: '审批成功',
                        type: 'success'
                    });
            ...
            changeImportStatus(row){
                this.$router.push({path: `/purchase/vendorRegisterAdd`,  query:{id: row.id}})  //跳转到add去完善信息
            },
