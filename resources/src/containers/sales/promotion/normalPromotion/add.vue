                    ...
                    <div>
                        <div class="displayStyle">
                            <el-form-item label="业务实体：">
                                <el-select v-model="normalPromotionForm.userGroupId"
                                    :multiple="false"
                                    filterable
                                    clearable
                                    size="small"
    ...
    export default {
        ...
        data() {
            ...
            return{
                ...
                explainTexter:'[数量]字段在每种促销类型下代表不同含义,如下:\n'+'"限时促销":不需要填写,默认0即可.\n'+'"批量促销":需要填写满足促销条件的最小批量值,需大于0,当销售的商品达到该批量时,所有该商品都按照优惠价(或优惠折扣)进行促销.例如:当某商品一次性购买10个,则按八折出售,则该字段填写10即可.\n'+'"偶数促销":需要让第几个执行促销则填写几.例如:希望第二个半价,则填写2,希望每第三个打八折,则填写3\n'+'[折扣]字段说明:当优惠类型为1-按优惠折扣时有效,指的是按照原价的百分之多少计算促销价格;例如:如果希望打八折则填写80,希望打九折则填写90',
            }
        },
        mounted(){
        ...
        methods:{
            ...mapActions(["userGroupAll","normalPromotionAdd","ContractPurchaseSkuList","QueryPurchaseOrderDetailList","showPageLoading","createProcureOrder","savePurchaseOrder","approvePurchaseOrder","hidePageLoading"]),
            // // 表格初始化
            addGoodsMsg(){
                ...
                this.$refs[normalPromotionForm].validate((valid) => {
                    if (valid) {
                        ...
                        if (goArr) {
                            ...
                            this.normalPromotionAdd(datas).then((purchaseId) =>{
                                this.OnBackLatePage();
                            })
                        }
