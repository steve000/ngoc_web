        ...
        <div>
            <div class="displayStyle">
                <el-form-item label="促销标题：">
                    <el-input v-model="combPromotionForm.promotionTitle" placeholder="请填写促销标题" class="row_width" size="small">
                    </el-input>
                </el-form-item>
            </div>
            <div class="displayStyle">
              <el-form-item label="促销类型：" prop="promotionType">
                <el-select
                  ...
                  v-model="combPromotionForm.promotionType"
                   @change='getChangePromotionType'
                  >
        ...
        <div>
            <div class="displayStyle">  
              <el-form-item label="业务实体：" prop="userGroupId">
                <el-select v-model="combPromotionForm.userGroupId"
                  :multiple="false"
                  filterable
                  clearable
                  size="small"
            ...
            <div class="displayStyle">       
              <el-form-item label="促销详情描述：">
                <el-input v-model="combPromotionForm.descriptions"
                  class="row_width" size="small" placeholder="促销详情描述"></el-input>
        ...
        <el-tab-pane label="商品清单" name="first">
          ...
          <el-table
            ...
            <el-table-column
              ...
              label="加价金额">
              <template slot-scope="scope">
                <el-input type='number' v-number-decimal-only size="small" v-model="scope.row.addTotal"></el-input>
              </template>
            </el-table-column>
          </el-table>            
        </el-tab-pane>
        <el-tab-pane label="促销结果" name="second">
          ...
          <el-table :data='resultList' style='width:300px'>
            ...
            <el-table-column widtn='120' prop="buyCount" label="数量">
              <template slot-scope="scope">
                <el-input :disabled='combPromotionForm.promotionType!=0' size="small" type='number' v-number-decimal-only v-model="scope.row.buyCount"></el-input>
              </template>
            </el-table-column>
            <el-table-column widtn='120' prop="buyTotal" label="金额">
              <template slot-scope="scope">
                <el-input type="number" size="small" :min="0" v-number-decimal-only v-model="scope.row.buyTotal"></el-input>
              </template>
            </el-table-column>
          </el-table>
...
export default {
  ...
  methods: {
    ...
    //
    getChangePromotionType(val) {
      this.combPromotionForm.promotionType = val;
      if (val == 0) {
      ...
      if (val == 0) {
        this.isWhether = true
        for (let i = 0; i < this.combPromotionList.length; i++) {
          if (this.combPromotionList[i].mustCount == '1' || this.combPromotionList[i].mustCount) {
            this.combPromotionList[i].mustCount = '0'
          }
        }
      } else if (val == 1) {
        this.isWhether = true
        for (let i = 0; i < this.combPromotionList.length; i++) {
          if (this.combPromotionList[i].mustCount == '0' || this.combPromotionList[i].mustCount) {
            this.combPromotionList[i].mustCount = '1'
          }
        }
      } else if (val == 2) {
    ...
    contractFilter2() {
      this.querySkuCodeByProductName().then(res => {
        ...
        console.log(this.productList, "查询结果");
      });
    },
    // 表格初始化
    addGoodsMsg() {
      ...
      this.$refs[combPromotionForm].validate((valid) => {
          if (valid) {
              ...
              if (goArr) {
                ...
                this.combPromotionAdd(params).then(res => {
                  this.$message({
                    type: "success",
                    message: `操作成功`
                  });
    ...
    //业务实体搜索
    groupFilter() {
      this.userGroupAll({ status: 1 }).then(res => {
