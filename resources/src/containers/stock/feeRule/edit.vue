                    ...
                    <div>
                        ...
                        <div class="displayStyle">
                            <el-form-item label="收款实体：" prop="recUserGroupId">
                                <el-select v-model="feeRuleAddForm.recGroupName"
                                    :multiple="false"
                                    filterable
                                    clearable
                                    size="small"
...
export default {
  ...
  mounted() {
    ...
    this.groupFilter();
    this.getStockFeeruleQueryFunction()
  },
  computed: {},
  methods: {
    ...mapActions([
    ...
    getStockFeeruleQueryFunction() {
      let obj = {
        id: this.$route.params.id
      }
      this.stockFeeruleQuery(obj).then(res => {
        this.feeRuleAddForm = res
      });
    },
    savePurchase(feeRuleAddForm) {
      this.$refs[feeRuleAddForm].validate((valid) => {
        if (valid) {
          ...
          this.stockFeeruleEdit(datas).then(purchaseId => {
            this.OnBackLatePage();
          });
        } else {
