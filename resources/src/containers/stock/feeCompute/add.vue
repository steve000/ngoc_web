                        ...
                        <div class="displayStyle">
                            <el-form-item label="支付实体：" prop="payUserGroupId">
                                <el-select v-model="feeComputeAddForm.payGroupName"
                                    ...
                                    @clear="clearGetPayGroupInfo"
                                    @change="getPayGroupInfo"
                        ...
                        <div class="displayStyle">
                            <el-form-item label="收款实体：" prop="recUserGroupId">
                                <el-select v-model="feeComputeAddForm.recGroupName"
                                    ...
                                    @clear="clearGetRecUserGroupInfo"
                                    @change="getRecUserGroupInfo"
                                    class="text-dash-show row_width"
                                    >
                                    <el-option
  ...
  methods: {
    ...
    savePurchase(feeComputeAddForm) {
      this.$refs[feeComputeAddForm].validate((valid) => {
        if (valid) {
          if (!this.feeComputeAddForm.detailList.length) {
            this.showWarning("保存失败，请先添加费用明细");
            return false;
          } else {
            let datas = {};
            datas = this.feeComputeAddForm;
            this.stockFeecomputeAdd(datas).then(purchaseId => {
              this.OnBackLatePage();
            });
          }
        } else {
    ...
    clearGetPayGroupInfo() {
      this.feeComputeAddForm.payUserGroupId = ''
      this.feeComputeAddForm.payGroupName = ''
    },
    getPayGroupInfo(val) {
    ...
    clearGetRecUserGroupInfo() {
      this.feeComputeAddForm.recUserGroupId = ''
      this.feeComputeAddForm.recGroupName = ''
      this.feeComputeAddForm.detailList = []
    },
    getRecUserGroupInfo(val) {
      ...
      if (num == this.recUserGroupList.length) {
        if (this.feeComputeAddForm.payUserGroupId && val) {
          let obj = {
