            ...
            <div class="viewLeftTop">
                <el-row>
                    <el-col :span="8">
                        <h4 class="order">
                            <span>退货单号：</span>
                            <span >{{billDetail.purchaseReturnNo}}</span>
                        </h4>
                    </el-col>
                    <el-col :span="8">
                        <h4 class="order">
                            <span>单据状态：</span>
                            <span>{{billDetail.status==0?'待发货':'已完成'}}</span>
                        </h4>
                    </el-col>
                    <el-col :span="8">
                        <h4 class="order">
                    ...
                    <el-col :span="24" v-show="billDetail.status==0 && billDetail.approveStatus==1">
                        <el-button type="text" class="mr15"
                                    @click="queryWaitStockInProductFunction()">出库
                        </el-button>
                    </el-col>
                </el-row>
            </div>
                        ...
                        <div class="stockPerInList">
                                <el-table :data="skuList">
                                    ...
                                    <el-table-column label="退货数量" prop="returnCount" min-width="110">
                                        <template slot-scope="scope">
                                            <div class="row-show" :title="scope.row.returnCount">{{scope.row.returnCount}}
                                            </div>
                                        </template>
                                    </el-table-column>
                                    <el-table-column label="退货价(元)" prop="purchasePrice" min-width="110">
                                        <template slot-scope="scope">
                                            <div class="row-show" :title="scope.row.purchasePrice">{{scope.row.purchasePrice}}
                                            </div>
                                        </template>
                                    </el-table-column>
                                    <el-table-column label="备注" prop="comment" min-width="110">
                                        <template slot-scope="scope">
        ...
        <div class="DetailsRightBox">
            ...
            <el-form :modle="billDetail" label-position="left" class="TotalBasic">
                ...
                <el-form-item label="退货金额：">
                    <span>{{billDetail.totalAmount ? billDetail.totalAmount : '--'}}</span>
                </el-form-item>
                <el-form-item label="仓库名称：">
                    <span>{{billDetail.storehouseName ? billDetail.storehouseName : '--'}}</span>
                </el-form-item>
                <el-form-item label="仓库类型：">
                    <span>{{getStoreType(billDetail.storehouseType) ? getStoreType(billDetail.storehouseType) : '--'}}</span>
                </el-form-item>
...
<script>
    ...
    export default {
        ...
        methods: {
            ...mapActions(["showPageLoading", "hidePageLoading", 'purchaseReturnBillQuery', 'purchaseReturnBillOutStock']),
            getRadioValue(value){
            },
            queryData(){
            ...
            queryWaitStockInProductFunction() {
                this.purchaseReturnBillOutStock({seqNo: this.billDetail.seqNo, purchaseReturnNo: this.billDetail.purchaseReturnNo}).then(res => {
                    console.log(res)
                    this.$message({
                        type: "success",
                        message: `退货单出库成功`
